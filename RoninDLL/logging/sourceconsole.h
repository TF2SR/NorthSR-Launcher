...
class SourceConsoleSink : public CustomSink
{
  private:
	std::map<spdlog::level::level_enum, SourceColor> m_LogColours = {
		{spdlog::level::trace, RN::Colors::TRACE.ToSourceColor()},
		{spdlog::level::debug, RN::Colors::DEBUG.ToSourceColor()},
		{spdlog::level::info, RN::Colors::INFO.ToSourceColor()},
		{spdlog::level::warn, RN::Colors::WARN.ToSourceColor()},
		{spdlog::level::err, RN::Colors::ERR.ToSourceColor()},
		{spdlog::level::critical, RN::Colors::CRIT.ToSourceColor()},
		{spdlog::level::off, RN::Colors::OFF.ToSourceColor()}};
